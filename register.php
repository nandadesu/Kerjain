...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["address"]))) {
        $adress_err = "Please enter your address";
    } else {
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        $sql = "INSERT INTO users (username, email, password, address, money) VALUES (?, ?, ?, ?, '100000')";

        if ($stmt = mysqli_prepare($connect, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssss", $param_username, $param_email ,$param_password, $param_address);
                ...
                <div class="wrapper">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($address_err)) ? 'has-error' : ''; ?>">
                            ...
                            <textarea name="address" class="form-control" value="<?php echo $address; ?>" cols="41" rows="3"></textarea>
